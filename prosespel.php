      ...
      while ($row=mysqli_fetch_array($harga)){
        ...
        if ($krg < 0){
          echo "<script>alert('Saldo Tidak Mencukupi Untuk Pembayaran Kamar');</script>";
           ...
           $update = $connection->query("update pelanggan set saldo = '" .$krg. "' where id = '" .$id. "' ");
          $insert = $connection->query("insert into pesanan (`ID_pelanggan`,`ID_resepsionis`,`tipe_kamar`,`jumlah`,`harga`,`tanggal`,`total`) values ('$id','$resepsionis','$tipe', '$jumlah', '$hrg', '$date', '$total')");
            if ($insert) {
                echo "<script>alert('Sukses tambah pesanan');location.href='home_pengguna.php';</script>";
            } else {
                echo "<script>alert('Gagal tambah pesanan');location.href='home_pengguna.php';</script>";
            }
